...
public:
  ...
  Ray ScreenToWorldRay(float mouseX, float mouseY, const glm::mat4 &view,
                       const glm::mat4 &projection, float screenWidth,
                       float screenHeight)
  {
  ...
  PickObject(float mouseX, float mouseY,
             const std::vector<shared_ptr<GameObject>> objects,
             const glm::mat4 &view, const glm::mat4 &projection,
             float screenWidth, float screenHeight)
  {
    Ray ray = ScreenToWorldRay(mouseX, mouseY, view, projection, screenWidth,
                               screenHeight);
  ...
  glm::vec3 PickGroundPosition(float mouseX, float mouseY,
                               const glm::mat4 &view,
                               const glm::mat4 &projection, float screenWidth,
                               float screenHeight, float groundY)
  {
    Ray ray = ScreenToWorldRay(mouseX, mouseY, view, projection, screenWidth,
                               screenHeight);

    // Check if ray is parallel to the ground plane (pointing horizontally)
    if (abs(ray.direction.y) < 0.0001f)
    {
      // Ray is parallel to ground, no intersection
      return glm::vec3(0.0f, groundY, 0.0f); // Return center point as fallback
    }

    // Calculate intersection with horizontal plane at y = groundY
    // Ray equation: P = origin + t * direction
    // Plane equation: y = groundY
    // Solve: origin.y + t * direction.y = groundY
    float t = (groundY - ray.origin.y) / ray.direction.y;

    // If t is negative, the intersection is behind the camera
    if (t < 0.0f)
    {
      return glm::vec3(0.0f, groundY, 0.0f); // Return fallback position
    }
